 ...
 */
// some parts of the file have been altered by Stephen Petrie, original article can be found at http://coding.smashingmagazine.com/2011/09/23/create-an-animated-bar-graph-with-html-css-and-jquery/
	...
	function createGraph(data, container) {
		...
		var tableData = {
			...
			yLegend: function() {
				...
				for (var i = 0; i < yAxisMarkings; i++) {
				    yLegend.unshift(Math.ceil(((chartYMax * i) / (yAxisMarkings - 1))) );
				}
				return yLegend;
			},
		...
		$.each(columnGroups, function(i) {
			...
			for (var j = 0, k = columnGroups[i].length; j < k; j++) {
				...
				var barObj = {};
				barObj.label = this[j];
				//current multiplied by 1000 for smaller numbers, 100 works for larger nubmers 500 or more (by spetrie)
				barObj.height = Math.floor(barObj.label/(yMarkings-1.5)) + '%';
